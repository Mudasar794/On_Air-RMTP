...
class WelcomeViewController: UIViewController {
    ...
    @IBAction func connectButton(_ sender: UIButton ) {
        //MARK:- Create room with device id
        Defaults[.server] = "ec2-34-232-76-94.compute-1.amazonaws.com"
        print("Ready to go")
        let url = Defaults[.server] ?? ""
        self.roomField.text = (UIDevice.current.identifierForVendor?.uuidString)
        let room = roomField.text!
        let stream = LFLiveStreamInfo()
        stream.url = "rtmp://\(url)/LiveApp/\(room)"
        //stream.url = "rtmp://ec2-34-232-76-94.compute-1.amazonaws.com/LiveApp/\(room)"
        ...
        if roomField.text!.count == 0 {
            ...
            let url = Defaults[.server] ?? ""
            //let room = roomField.text!
            let room = (UIDevice.currentDevice().identifierForVendor.UUIDString)
            let stream = LFLiveStreamInfo()
            stream.url = "rtmp://\(url)/LiveApp/\(room)"
            //stream.url = "rtmp://ec2-34-232-76-94.compute-1.amazonaws.com/LiveApp/\(room)"
            session.startLive(stream)
    ...
    @IBAction func PressSteambtn(_ sender: UIButton) {
        
        let videoURL = URL(string: "rtmp://ec2-34-232-76-94.compute-1.amazonaws.com/LiveApp/test")
    ...
    @IBAction func serverTapped(_ sender: UIButton) {
        AlertHelper.getInstance().addOption("Ok", onSelect: {
            (address) in
            if (address!.count > 0) {
                self.serverButton.setTitle("Invalid Name", for: .normal)
            } else {
                self.serverButton.setTitle("Invalid Name", for: .normal)
            }
        })
